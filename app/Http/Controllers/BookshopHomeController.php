...
class BookshopHomeController extends Controller
{
    ...
    {
        # Home page Books
        $thriller = Book::with('category')->whereHas('category', function($query) {
            $query->where('slug', 'thriller'); })
            ->take(8)
            ->latestFirst()
            ->get();
        $selfhelp = Book::with('category', 'author', 'image')
            ->whereHas('category', function ($query){
                $query->where('slug', 'selfhelp'); })
            ->take(4)
            ->latestFirst()
            ->get();
        $discount_books = Book::with('category')
            ...
            ->take(6)
            ->get();
        return view('public.home', compact('thriller', 'discount_books', 'selfhelp'));
    }
